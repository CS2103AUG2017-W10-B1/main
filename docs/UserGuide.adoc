...
=== Viewing quick help : `quickhelp` (Since v1.2)
...
[TIP]
A person can have any number of tags, phone numbers, emails or addresses (including 0) (Since v1.2)
...
Finds persons whose names match the regex given. Note that only the name is searched. (Since v1.2) +
...
=== Changing a person's remark : `remark`
...
View a monthly breakdown on the number of persons added to the address book. +
Format: `statistics` +
Alias: `st`
