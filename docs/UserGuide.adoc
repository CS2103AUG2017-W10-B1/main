...
PIMP (Personal Information Manager Program) is a command line interface (CLI) address book that allows you to store your contacts. You can use PIMP to add friends, add specific tags to them, add their social media accounts (such as Facebook, Twitter, Instagram), or add remarks. You can search through the list of your contacts by typing specific names. For more advanced user, you can use regex to find your friends. Most of the functionality works by typing in single line commands.
...
These are the more advanced commands. Functionality here is useful for the user, but not essential. 
...
==== Accessing a person's social media : `socialmedia` (Coming v2.0)
...
At the left side of the screen where the addressbook entries are displayed, we can see an "Accesses: #number" row, which describes the number of times this entry has been accessed. Clicking on this entry, or modifying it using any of the edit, add/remove tag functions will increase this counter.
